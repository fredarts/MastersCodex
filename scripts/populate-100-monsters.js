...
const PORTUGUESE_TO_ENGLISH_MAP = {
  "Aboleth": "aboleth",
  "Alma Penada": "wraith",
...
// Habilitar suporte nativo ao fetch do Node 18+
async function fetchMonsterFromAPI(slug) {
...
async function run() {
  ...
  for (const file of files) {
    ...
    if (CUSTOM_FALLBACKS[slug]) {
      ...
      rawMonster = { ...CUSTOM_FALLBACKS[slug], name: displayName };
    } else {
      // B. Buscar da API
      try {
        ...
        let speedText = '9m (30ft)';
        if (apiData.speed) {
          speedText = Object.entries(apiData.speed)
            .map(([k, v]) => {
              const val = String(v);
        ...
        const abilities = (apiData.special_abilities || []).map((sa) => ({
          name: translateText(sa.name),
          desc: translateText(sa.desc)
        }));

        const actions = (apiData.actions || []).map((act) => ({
        ...
        console.warn(`   -> Falha na API para "${displayName}" (Slug: ${slug}). Usando valores padrões. Detalhe: ${error.message}`);
