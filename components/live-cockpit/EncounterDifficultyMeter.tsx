...
export const EncounterDifficultyMeter: React.FC<EncounterDifficultyMeterProps> = ({
  ...
  return (
    <div className="bg-[#0b0f19] border border-amber-500/30 rounded-2xl p-3.5 shadow-xl text-slate-200 space-y-3 select-none">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div
            className={`flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-black uppercase tracking-wider font-serif border ${getDifficultyBadgeStyle()}`}
          >
            {getDifficultyIcon()}
          ...
          {result.monsterCount > 0 && (
            <button
              ...
              className={`text-[11px] font-mono px-2 py-0.5 rounded-lg border flex items-center gap-1 transition-all cursor-pointer ${
                isMonsterListOpen
            ...
            >
              <span>👹 {result.monsterCount} {result.monsterCount === 1 ? 'Monstro' : 'Monstros'} (Mult. {result.multiplier}x)</span>
              {isMonsterListOpen ? <ChevronUp className="w-3 h-3 text-rose-400" /> : <ChevronDown className="w-3 h-3 text-slate-400" />}
            </button>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-900/80 hover:bg-slate-800/90 border border-slate-700/80 rounded-xl text-[11px] font-mono text-slate-300 transition-all cursor-pointer"
          ...
          >
            ...
            {isPartyConfigOpen ? <ChevronUp className="w-3 h-3 text-slate-500" /> : <ChevronDown className="w-3 h-3 text-slate-500" />}
          </button>
        </div>
      </div>
      ...
      {isMonsterListOpen && monsters.length > 0 && (
        <div className="p-2.5 bg-[#121829] border border-rose-500/30 rounded-xl space-y-2 animate-in fade-in duration-150">
          <div className="flex items-center justify-between text-[11px]">
            <span className="font-bold text-rose-300 flex items-center gap-1">
              <Swords className="w-3 h-3 text-rose-400" /> Monstros no Encontro Atual ({monsters.length}):
            </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 max-h-36 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-700">
            {monsters.map((m, idx) => (
              <div
                key={m.id || `${m.name}-${idx}`}
                className="flex items-center justify-between gap-2 px-2 py-1 bg-slate-950/80 border border-slate-800/80 rounded-lg text-xs"
              >
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className="font-semibold text-slate-200 truncate max-w-[120px]" title={m.name}>
                    {m.name || `Monstro #${idx + 1}`}
                  </span>
                  <span className="text-[10px] font-mono text-rose-400 bg-rose-950/40 px-1 py-0.2 rounded border border-rose-900/50 shrink-0">
                    CR {m.cr || '?'}
                  </span>
                </div>
      ...
      {isPartyConfigOpen && (
        <div className="p-2.5 bg-[#121829] border border-indigo-500/30 rounded-xl space-y-2 animate-in fade-in duration-150">
          <div className="flex items-center justify-between text-[11px]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] text-slate-400 block mb-1">Nº de Jogadores:</label>
              <div className="flex items-center gap-1 bg-slate-950 border border-slate-800 rounded-lg p-1">
                <button
                  type="button"
                  onClick={() => setCustomPartySize(Math.max(1, (customPartySize ?? result.partySize) - 1))}
                  className="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded text-xs font-bold text-white flex items-center justify-center cursor-pointer"
                >
                  -
                </button>
                <span className="flex-1 text-center font-mono font-bold text-xs text-white">
            ...
            <div>
              <label className="text-[10px] text-slate-400 block mb-1">Nível Médio:</label>
              <div className="flex items-center gap-1 bg-slate-950 border border-slate-800 rounded-lg p-1">
                <button
                  type="button"
      ...
      <div className="space-y-1.5">
        <div className="relative h-3 bg-slate-950 rounded-full border border-slate-800 overflow-hidden flex shadow-inner">
          ...
          <div
            className={`absolute top-0 left-0 bottom-0 transition-all duration-500 rounded-full ${
              result.difficulty === 'deadly'
                ? 'bg-gradient-to-r from-emerald-500 via-amber-500 to-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.8)]'
                : result.difficulty === 'hard'
                ? 'bg-gradient-to-r from-emerald-500 via-amber-500 to-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.8)]'
                : result.difficulty === 'medium'
                ? 'bg-gradient-to-r from-emerald-500 to-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.6)]'
                : result.difficulty === 'easy'
                ? 'bg-gradient-to-r from-cyan-500 to-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]'
                : 'bg-cyan-500/80 shadow-[0_0_6px_rgba(6,182,212,0.6)]'
            }`}
      ...
      <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-800/80 text-[11px] font-mono">
        <div className="bg-[#121726] p-2 rounded-xl border border-slate-800/80 flex items-center justify-between">
          <span className="text-slate-400 flex items-center gap-1">
            <Zap className="w-3.5 h-3.5 text-amber-400" /> XP Ajustado:
          </span>
        ...
        <div className="bg-[#121726] p-2 rounded-xl border border-slate-800/80 flex items-center justify-between">
          <span className="text-slate-400 flex items-center gap-1">
            <Award className="w-3.5 h-3.5 text-emerald-400" /> Recompensa:
          </span>
          <strong className="text-emerald-300 text-xs font-bold" title={`${result.xpPerPlayer.toLocaleString()} XP por jogador`}>
            {result.totalRawXp.toLocaleString()} XP <span className="text-[9px] text-slate-400 font-normal">({result.xpPerPlayer.toLocaleString()}/PC)</span>
          </strong>
        </div>
      </div>
