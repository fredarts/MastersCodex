...
export const SceneDungeonMapsStudio: React.FC<SceneDungeonMapsStudioProps> = ({
  ...
  return (
    <div className="w-full h-full min-h-0 flex-1 flex flex-col lg:flex-row gap-3 xl:gap-4 max-w-[1720px] mx-auto overflow-hidden animate-fade-in select-none">
      ...
      {/* ========================================================================= */}
      {/* COLUNA ESQUERDA: LISTA DE MASMORRAS DA CAMPANHA                           */}
      {/* ========================================================================= */}
      <div className="bg-[#121824] rounded-2xl border border-[#2a3449] p-3 sm:p-3.5 shadow-2xl flex flex-col gap-2.5 w-full lg:w-[35%] xl:w-[32%] h-full min-h-0 overflow-hidden flex-shrink-0">
        {/* Header da Lista */}
        <div className="flex items-center justify-between border-b border-[#2a3449] pb-2 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-xs sm:text-sm font-bold text-slate-100">Masmorras da Campanha</h3>
              <p className="text-[10px] text-emerald-400 font-mono">
                {associatedMapIds.length} associada(s) à cena ativa
              </p>
            </div>
          </div>
          <span className="text-[11px] font-mono font-bold bg-[#0a0d14] text-slate-300 border border-[#2a3449] px-2 py-0.5 rounded-lg shadow-inner">
            {campaignMaps.length} total
          </span>
        </div>

        {/* Lista de Cards de Masmorras (SCROLL INTERNO) */}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2.5">
                  ...
                  <div className="relative w-14 h-10 rounded-lg bg-black overflow-hidden border border-[#2a3449] shrink-0 shadow">
                    {cover ? (
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded-lg text-[9.5px] font-bold transition-all flex items-center gap-1 shrink-0 cursor-pointer ${
                      isAssociated
      ...
      {/* ========================================================================= */}
      {/* COLUNA DIREITA: SHOWCASE & DOSSIER DA MASMORRA                            */}
      {/* ========================================================================= */}
      <div className="bg-[#121824] rounded-2xl border border-[#2a3449] p-3 sm:p-3.5 shadow-2xl flex flex-col gap-2.5 w-full lg:w-[65%] xl:w-[68%] h-full min-h-0 flex-1 overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-[#2a3449] pb-2 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-xl text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer shadow-md ${
                isCurrentAssociated
            ...
            {onEditMapInMapMaker && (
              <button
                ...
                className="px-2.5 py-1 bg-[#161f30] hover:bg-[#202c44] border border-amber-500/40 hover:border-amber-400 text-amber-300 font-bold text-[11px] rounded-xl shadow flex items-center gap-1 cursor-pointer active:scale-95 transition-all"
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 flex-1 min-h-0 overflow-hidden">
          ...
          <div className="md:col-span-5 flex flex-col gap-2 min-h-0 overflow-hidden">
            <label className="text-[9.5px] font-bold text-slate-400 uppercase font-mono tracking-wider flex items-center gap-1 shrink-0">
              <Map className="w-3 h-3 text-amber-400" />
              Capa Cinemática da Masmorra:
            </label>

            {/* Container da Capa com Aspect Ratio e Moldura */}
            <div className="relative w-full aspect-[16/10] max-h-[160px] sm:max-h-[175px] bg-black/90 rounded-xl overflow-hidden border border-amber-500/40 shadow-xl flex items-center justify-center group shrink-0">
              {currentCover ? (
                <img
                ...
                <div className="p-4 text-center text-slate-500 flex flex-col items-center gap-1.5">
                  <Map className="w-8 h-8 opacity-30 text-amber-400" />
                  <p className="text-[11px] font-semibold">Nenhuma capa cadastrada</p>
                  <p className="text-[8.5px] text-slate-600">Configure no Map Maker</p>
                </div>
              )}
              ...
              <div className="absolute bottom-1.5 left-2.5 right-2.5 flex items-center justify-between text-[9.5px] font-mono font-bold text-amber-300">
                <span className="truncate">{currentMap?.title || 'Sem Título'}</span>
                <span className="bg-black/70 px-1 py-0.2 rounded border border-amber-500/30">{currentChallengeRating}</span>
              </div>
            </div>

            {/* Estatísticas Rápidas da Masmorra */}
            <div className="p-2.5 bg-[#0a0d14] rounded-xl border border-[#2a3449] space-y-1 text-[10.5px] font-mono shrink-0">
          ...
          <div className="md:col-span-7 flex flex-col gap-2 min-h-0 h-full overflow-hidden">
            
            {/* Título da Masmorra */}
            <div className="p-2 bg-[#0a0d14] border border-[#2a3449] rounded-xl space-y-0.5 shrink-0">
            ...
            <div className="p-2 bg-[#0a0d14] border border-[#2a3449] rounded-xl space-y-0.5 shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                <span className={`text-[11px] font-bold px-2 py-0.2 rounded-lg border font-mono ${currentTierPreset.color}`}>
                  {currentTierPreset.label} • {currentChallengeRating}
                </span>
              </div>
            </div>

            {/* Lore & Descrição Narrativa da Masmorra (Scroll Interno com Altura Flexível) */}
            <div className="p-2.5 bg-[#0a0d14] border border-[#2a3449] rounded-xl flex-1 flex flex-col space-y-1 min-h-0 overflow-hidden shadow-inner">
              <span className="text-[9px] font-bold text-slate-400 uppercase font-mono tracking-wider flex items-center gap-1 shrink-0">
                <BookOpen className="w-3 h-3 text-amber-400" />
                Lore, Rumores & Descrição Narrativa (Leitura do Mestre):
              </span>
              <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar text-xs text-slate-200 leading-relaxed pr-1 font-serif">
                {currentDescription ? (
