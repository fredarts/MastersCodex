...
import { Combatant, CharacterCurrency, CharacterEquipmentItem } from '@/lib/types';
...
export const ContainerLootModal: React.FC<ContainerLootModalProps> = ({
  ...
  const handleClaimItemForCharacter = (itemIndex: number) => {
    ...
    toast.success(`"${itemToClaim}" foi coletado por ${selectedCharacterName}!`);
  };
  ...
  const handleSendItemToPartyLoot = async (itemIndex: number) => {
    ...
    toast.success(`"${itemToSend}" foi enviado para o Baú do Grupo!`);
  };
  ...
  const handleSendAllToPartyLoot = async () => {
    ...
    const partyItems = items.map((itemName, idx) => ({
      id: `party-item-${Date.now()}-${idx}`,
      name: itemName,
      quantity: 1,
      weight: '1 lb',
    ...
    toast.success(`✨ Todo o saque foi enviado com sucesso para o Baú do Grupo!`);
    onClose();
  };
      ...
      <div className="relative w-full max-w-lg bg-slate-950 border border-amber-500/40 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Colete os itens para seu personagem ou envie para o Baú do Grupo.
              </p>
            </div>
          </div>
                ...
                <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2.5">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    >
                      ...
                      <span>Pegar para {selectedCharacterName}</span>
                    </button>
                    <button
                      type="button"
              ...
              {currentLoot.items && currentLoot.items.length > 0 && (
                <div className="space-y-2">
                  <span className="font-bold text-slate-300 flex items-center gap-1.5">
                    <Package className="w-4 h-4 text-cyan-400" /> Itens e Tesouros:
                  </span>

                  <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                    {currentLoot.items.map((item, idx) => (
                      <div
                        key={idx}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            title="Enviar para Baú do Grupo"
                          >
                            Baú do Grupo
                          </button>
                        </div>
        ...
        <div className="flex items-center justify-between px-6 py-3.5 bg-slate-900/80 border-t border-slate-800">
          ...
          {hasAnyLootLeft && (
            ...
            >
              <Users className="w-3.5 h-3.5" />
              <span>Pegar Tudo para o Grupo (Receber Loot)</span>
            </button>
          )}
        </div>
      </div>
