    ...
    activeCombatants.forEach((c) => {
      if (!targetFilter || matchesQuery(c.name, targetFilter)) {
        ...
        results.push({
          ...
          subtitle: `PV: ${c.currentHp}/${c.maxHp} | CA: ${c.ac} | Condições: ${c.conditions?.length ? c.conditions.join(', ') : 'Nenhuma'}`,
          badge: c.isPlayer ? 'JOGADOR' : 'MONSTRO',
          iconType: 'Shield',
  ...
  if (monsterFilter && (isMonsterQuery || monsterFilter.length >= 3)) {
    ...
    matchedMonsters.forEach((monster) => {
      results.push({
        ...
        subtitle: `${monster.size} ${monster.type} • CA ${monster.ac} • PV ${monster.hp} (${monster.hit_dice || 'D6'}) • Vel: ${monster.speed}`,
        badge: `ND ${monster.cr}`,
        iconType: 'Skull',
  ...
  if (itemFilter && (isItemQuery || itemFilter.length >= 3)) {
    ...
    matchedItems.forEach((item) => {
      results.push({
        ...
        subtitle: `Custo: ${item.cost} • Peso: ${item.weight} kg • ${item.description.slice(0, 75)}...`,
